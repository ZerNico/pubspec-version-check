...
const pubspecFileName = core.getInput('file-name') || 'pubspec.yaml',
...
interface PubspecObj {
  version: string
}
function isPubspecObj(value): value is PubspecObj {
...
async function checkDiff(sha: string, version: string) {
  try {
    ...
    const pkg = commit.files.find(f => f.filename == pubspecFileName)
    if (!pkg) return false
...
async function processDirectory(dir: string, commits: Commit[]) {
  try {
    const pubspecFile = join(dir, pubspecFileName),
      pubspecObj = await readYaml(pubspecFile).catch(() => {
        Promise.reject(
          new NeutralExitError(`Pubspec file not found: ${pubspecFile}`)
        )
      })
    ...
    await checkCommits(commits, pubspecObj.version)
  } catch (e) {
    core.setFailed(e)
  }
}
